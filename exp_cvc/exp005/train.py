...
def forward(data, model, device, criterion, mode="train"):
    ...
    loss = loss_label * 0.5 + loss_mask * 0.5
...
if __name__ == "__main__":
    ...
    for cv in cv_list:
        ...
        val_dataset = RanzcrCVCDataset5Eval(df=df_val, image_size=img_size,
                                    image_folder=img_dir, from_image_folder=True,
                ...
                for step_train, data in enumerate(progress_bar):
                    if debug:
                        if step_train == 20:
                            break
                    with autocast():
                        loss, pred, targets, pred_labels, *_ = forward(data, model, device, criterion)
