...
from src.dataset import RanzcrSegDataset as RanzcrDataset
...
if __name__ == "__main__":
    ...
    for cv in cv_list:
        ...
        print('length dataset & dataloader')
        ...
        for e in range(start_epoch , start_epoch + n_epochs):
            ...
            if e > 0:
                ...
                writer.add_scalar(f"Loss_seg/train_cv{cv}", np.mean(losses_train), global_step=e)
            ...
            writer.add_scalar(f"Loss_seg/eval_cv{cv}", np.mean(losses_eval), global_step=e)
            ...
            if True: # e % 1 == 0 or e == 0:
                ...
                sample_mask = data['target'].cpu().numpy()
                sample_pred = pred_labels
            ...
            if best > np.mean(losses_eval):
                LOGGER.info(f'Best score update: {best:.5f} --> {np.mean(losses_eval):.5f}')
                best = np.mean(losses_eval)
                best_epoch = e
